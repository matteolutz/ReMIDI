#ifndef REMIDI_MESSAGE_H
#define REMIDI_MESSAGE_H

#include "ReMIDITypes.h"

enum class ReMIDIMessageType : uint8_t
{
...
struct ReMIDIMessage
{
    ReMIDIMessageType type;
    uint8_t data1;
    uint8_t data2;

    bool isValid() const
    {
        return type != ReMIDIMessageType::InvalidType;
    }

    static ReMIDIMessage createInvalidMessage()
    {
        return {ReMIDIMessageType::InvalidType, 0, 0};
    }

    static ReMIDIMessage createProgramChangeMessage(uint8_t programNumber)
    {
        return {ReMIDIMessageType::ProgramChange, programNumber, 0};
    }
};

#endif // REMIDI_MESSAGE_H